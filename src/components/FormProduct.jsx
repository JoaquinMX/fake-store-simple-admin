...
export default function FormProduct({ setOpen, setAlert, product }) {
  ...
  const handleSubmit = (event) => {
    ...
    if (product) {
      updateProduct(product.id).then((data) => {
        console.log(data);
        ...
        <div className="px-4 py-5 bg-white sm:p-6">
          <div className="grid grid-cols-6 gap-6">
            ...
            <div className="col-span-6 sm:col-span-3">
              ...
              <input defaultValue={product?.category ?? ""} type="text" name="category" id="category" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
            </div>
            ...
            <div className="col-span-6 sm:col-span-3">
              ...
              <input defaultValue={product?.image ?? ""} type="text" name="image" id="image" className="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md" />
            </div>
          </div>
        </div>
